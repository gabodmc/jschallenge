  ...
  const datos = async () => {
    await fetch('http://localhost:3001/api/movements/')
        ...
        .then((result) => {
                ...
                setTopTen(result.transactions.reverse().slice(result.length, 10));
                setIncomes(
                  result.transactions.filter((movement) => movement.revenue === 1)
                );
                setOutcomes(
                  result.transactions.filter((movement) => movement.revenue === 2)
                );
